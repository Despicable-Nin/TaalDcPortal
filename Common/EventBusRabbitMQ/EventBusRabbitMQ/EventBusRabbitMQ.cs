...
public class EventBusRabbitMq : IEventBus,  IDisposable
{
    ...
    public void Publish(IntegrationEvent @event)
    {
        ...
        var policy = RetryPolicy.Handle<BrokerUnreachableException>()
            .Or<SocketException>()
            .WaitAndRetry(_retryCount, r => TimeSpan.FromSeconds(Math.Pow(2, r)), (ex, time) =>
            {
    ...
    public void Subscribe<T, H>() where T : IntegrationEvent where H : IIntegrationEventHandler<T>
    {
        var eventName = _subsManager.GetEventKey<T>();
        DoInternalSubscription(eventName);

        _logger.LogInformation("Subscribing to event {EventName} with {EventHandler}", eventName, typeof(H).GetGenericTypeName());

        _subsManager.AddSubscription<T, H>();
        StartBasicConsume();
    }
